...
# Cookies de sessão funcionarem em cross-origin durante o dev
...
# Funções auxiliares para carregar os dados atualizados dos arquivos JSON
def load_users():
    global users
    with open(USERS_FILE, 'r', encoding='utf-8') as f:
        try:
...
def criar_usuario():
    load_users()  # Sempre carregar a versão mais recente dos usuários
...
def editar_usuario(user_id):
    load_users()      # Carrega dados atualizados dos usuários
    load_complaints() # Carrega dados atualizados das reclamações (necessário para atualizar autor)
...
def criar_reclamacao():
    load_complaints()  # Carrega dados atualizados das reclamações
    if 'user_id' not in session:
        return jsonify({"ok": False, "data": None, "error": {"code": 401, "message": "Usuário não está logado."}}), 401
    data = request.get_json()
...
def deletar_reclamacao(complaint_id):
    load_users()       # Carrega dados atualizados dos usuários (para atualizar favoritos)
    load_complaints()  # Carrega dados atualizados das reclamações
    if 'user_id' not in session:
        return jsonify({"ok": False, "data": None, "error": {"code": 401, "message": "Usuário não está logado."}}), 401
